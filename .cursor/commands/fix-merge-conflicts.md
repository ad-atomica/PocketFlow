...
1. Detect conflicts
   ...
   - Collect files with unmerged entry codes (DD, AU, UD, UA, DU, AA, UU) or containing conflict markers (<<<<<<<, =======, >>>>>>>).
2. Resolve conflicts per file
   ...
   - Language-aware strategy:
     - Lock files (package-lock.json, yarn.lock, pnpm-lock.yaml): prefer regenerating the appropriate lockfile via the package manager rather than manual edits; only keep merge changes conservatively if regeneration is not possible.
...
3. Validate
   - If Node/TypeScript/JS present: install deps if manifests changed (npm ci or npm install; yarn install --no-frozen-lockfile; pnpm install --no-frozen-lockfile), then run lint/typecheck/build/tests if available.
