...
class Chain:
    ...
    def __repr__(self):
        ...
        info = tmp.format(
            self.chain, len(self.residues), self.get_coords.shape[0], self.get_heavy_coords.shape[0]
        )
        return f"{self.__class__.__name__}({info})"
...
class Protein:
            ...
            if ix != 0:
                if res.idx - res_list[ix - 1].idx == 1 and res.chain == res_list[ix - 1].chain:
                    bond_idx_between_res = np.array(
                        [[N_term_ix, C_term_list[ix - 1]], [C_term_list[ix - 1], N_term_ix]], dtype=np.int64
                    )
    ...
    def get_atom_dict(self, removeHs=True, get_surf=False):
        atom_dict = {"element": [], "pos": [], "is_backbone": [], "atom_name": [], "atom_to_aa_type": []}
        ...
        if get_surf:
            atom_dict["surface_mask"] = np.array(
                [a.is_surf for a in self.get_heavy_atoms], dtype=bool
            )  # self.get_surf_mask()
