...
def change_features_of_neigh(
    ligand_feature_full, new_num_neigh, new_num_valence, ligand_atom_num_bonds, num_atom_type=7
):
...
def get_complex_graph(
    data, len_ligand_ctx, len_compose, num_workers=1, graph_type="knn", knn=16, radius=10.0
):
    ...
    idx_edge = torch.tensor(
        [a.squeeze() if len(a) > 0 else torch.tensor(-1) for a in idx_edge], dtype=torch.long
    )
    data.cpx_edge_type = torch.zeros(
        len(data.cpx_edge_index[0]), dtype=torch.long
...
def get_knn_graph(
    pos, k=16, edge_feat=None, edge_feat_index=None, num_workers=8, graph_type="knn", radius=5.5
):
    if graph_type == "rad":
...
def get_tri_edges(edge_index_query, pos_query, idx_ligand, ligand_bond_index, ligand_bond_type):
    ...
    index_real_cps_edge_i = torch.cat(index_real_cps_edge_i_list, dim=0).to(
        pos_query.device
