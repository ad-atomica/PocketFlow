...
class FocalMaker:
    ...
    def run(self, data):
        if data.ligand_context_pos.size(0) == 0:
            ...
            focal_idx_in_context = torch.norm(
            ...
            data.atom_label = torch.nonzero(
                data.ligand_element[data.masked_idx[0]] == self.atomic_numbers
            ).squeeze(0)
            ...
            focal_idx_in_context_mask = (data.context_idx.unsqueeze(1) == candidate_focal_idx_in_context).any(
                1
            )
...
class AtomComposer:
    ...
    def run(self, data):
        ...
        data.idx_protein_in_cpx = (
            torch.arange(num_protein_atom, dtype=torch.long) + num_ligand_ctx_atom
        )  # can be delete
...
COLLATE_KEYS = [  #'context_idx', 'masked_idx', 'ligand_masked_element', 'ligand_masked_pos',
    ...
    "focal_idx_in_context_",
    "cpx_edge_index",
    "cpx_edge_type",
    "cpx_edge_feature",
    "pos_query_knn_edge_idx_0",
    "pos_query_knn_edge_idx_1",
    "focal_label",
    "ligand_frontier",
    "y_pos",
    "edge_label",
    ...
    "pos_fake_knn_edge_idx_1",
    "pos_real",
    "pos_real_knn_edge_idx_0",
    "pos_real_knn_edge_idx_1",
