...
encoder_cfg = EasyDict({"edge_channels": 8, "num_interactions": 6, "knn": 32, "cutoff": 10.0})
focal_net_cfg = EasyDict({"hidden_dim_sca": 32, "hidden_dim_vec": 8})
atom_flow_cfg = EasyDict({"hidden_dim_sca": 32, "hidden_dim_vec": 8, "num_flow_layers": 6})
pos_predictor_cfg = EasyDict({"num_filters": [64, 64], "n_component": 3})
pos_filter_cfg = EasyDict({"edge_channels": 8, "num_filters": [32, 16]})
...
class PocketFlow(nn.Module):
    ...
    def get_loss(self, data):
        ...
        pos_query_knn_edge_idx = torch.stack([data.pos_query_knn_edge_idx_0, data.pos_query_knn_edge_idx_1])
        ...
        out_dict = {
            "loss": loss,
            "loss_atom": ll_atom,
            "loss_edge": ll_edge,  #'loss_fake':loss_fake, 'loss_real':loss_real,
            "loss_pos": loss_pos,
